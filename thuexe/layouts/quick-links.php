<div class="col-md-3 phai">
    <!-- <div id="fanpage" >
       <div id="fb">
           <h4 style="border-bottom:3px double orange;"><i class="fas fa-motorcycle"></i>Fanpage facebook</h4>
       </div>

       </div> -->
    <div id="xemnhieu" >
        <div id="hot">
            <h4 style="border-bottom:3px double orange;"> Xem nhiều nhất</h4>
        </div>
        <div style="padding-left: 10px">
            <h4><a href="<?php echo base_url()?>bao-gia.php"><i class="fas fa-hand-holding-usd"></i> Báo giá</a></h4>
            <hr>
            <h4><a href="kinh-nghiem.php"><i class="fas fa-star"></i> Kinh nghiệm</a></h4>
            <hr>
            <h4><a href="hop-dong.php"><i class="fas fa-paperclip"></i> Hợp đồng</a></h4>
            <hr>
            <h4><a href="<?php echo base_url()?>gioi-thieu.php"><i class="far fa-address-card"></i> Giới thiệu</a></h4>
            <hr>
            <h4><a href="lien-he.php"><i class="fas fa-phone-volume"></i> Liên hệ</a></h4>
        </div>
    </div>
    <div id="hotro" >
        <div id="ht">
            <h4 style="border-bottom:3px double orange;"><i class="fas fa-phone-square"></i> Hỗ trợ trực tuyến</h4>
        </div>
        <img src="<?php echo base_url()?>public/image/support.jpg" />
        <p>Hot line:</p>
        <p style="color: red">01673102897</p>
        <p>Email :</p>
        <p  style="color: red">Minhthang1609@gmail.com</p>
    </div>
</div>